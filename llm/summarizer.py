...
# ─────────────────────────────────────────────────────────────────────────────
# TASK-SPECIFIC OPTIONS + SYSTEM MESSAGES
# ─────────────────────────────────────────────────────────────────────────────

_TASK_OPTIONS: dict[str, dict] = {
    "summary":    {},          # uses model-specific _MODEL_OPTIONS — set below
    "guidance":   {"temperature": 0.1, "num_predict": 300,  "top_p": 1.0},
    "enrichment": {"temperature": 0.3, "num_predict": 400,  "top_p": 0.9},
    "classifier": {"temperature": 0.0, "num_predict": 200,  "top_p": 1.0},
}
...
_TASK_SYSTEM: dict[str, str] = {
    "summary": (
        ...
        "Always fill every section of the output format completely."
    ),
    "guidance": (
...
async def call_llm(
    ...
    Args:
        ...
        source_type: Used for model selection (github_repo → deepseek, etc.)
        task:        "summary" | "guidance" | "enrichment" | "classifier"
                     Controls system message and temperature.
    ...
    # Task-specific options override model defaults for non-summary tasks
    if task == "summary":
        options = _MODEL_OPTIONS.get(model, _DEFAULT_OPTIONS)
    else:
        options = _TASK_OPTIONS.get(task, _DEFAULT_OPTIONS)

    system = _TASK_SYSTEM.get(task, _TASK_SYSTEM["summary"])

    logger.info(f"[LLM] model={model}  source={source_type or 'default'}  task={task}")
    print(f"  [LLM] model={model}  source={source_type or 'default'}  task={task}")

    payload = {
        "model":   model,
        "system":  system,    # ← system message per task
        "prompt":  prompt,
        "stream":  False,
        "options": options,   # ← temperature per task
    }
...
async def summarize_data(data: dict) -> str:
    ...
    needs_chunking = any(
        isinstance(data.get(f), str) and len(data.get(f, "")) > 6000
        for f in _LONG_FIELDS
    )

    if not needs_chunking:
        prompt = build_summary_prompt(data)
        return await call_llm(prompt, source_type, task="summary")          # ← await
    ...
    return await call_llm(
        build_merge_prompt(partial_summaries, source_type=source_type),
